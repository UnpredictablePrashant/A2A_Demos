...
class DockermasterAgentExecutor(AgentExecutor):
    def __init__(self) -> None:
        self.model = os.environ.get("OPENAI_MODEL", "gpt-4.1-mini")
        api_key = os.environ.get("OPENAI_API_KEY", "")
        self.client = AsyncOpenAI(api_key=api_key) if api_key else None
        self.db_path = os.environ.get("DOCKERMASTER_DB_PATH", os.path.join(os.path.dirname(__file__), "dockermaster_tasks.db"))
        self._init_db()
    ...
    def _init_db(self) -> None:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS dockermaster_tasks (
    ...
    def _insert_task(self, input_text: str) -> tuple[int, float]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cur = conn.execute(
                """
                INSERT INTO dockermaster_tasks (input_text, status, result_text, error_text, created_at, updated_at, completed_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
        ...
        if not self.client:
            ...
            try:
                res = await self.client.responses.create(
                    model=self.model,
                    input=[
                        {"role": "system", "content": "You are Dockermaster Agent."},
